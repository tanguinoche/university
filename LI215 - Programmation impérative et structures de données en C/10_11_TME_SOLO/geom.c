...
/***************************************************/
void EcritForme(Un_elem *e, void *fic)
{
  if(e == NULL) {
    puts("Element null. (EcritForme)\n");
    return;
  }
  FILE *fch = (FILE *)fic;
  if(fch == NULL) {
    puts("fichier incorrect. (EcritForme)\n");
    return;
  }
  Une_forme *f = (Une_forme*)(e->forme);
  if(f->type == SEG) fprintf(fch, "S %c %c %c %u %u\n", f->coul.r, f->coul.v, f->coul.b, f->tabp[0]->id, f->tabp[1]->id);
  if(f->type == COURB) fprintf(fch, "C %c %c %c %u %u %u %u\n", f->coul.r, f->coul.v, f->coul.b, f->tabp[0]->id, f->tabp[1]->id, f->tabp[2]->id, f->tabp[3]->id);
  
}
/***************************************************/
void ItererElem(Un_elem *e, void (*fonc)(Un_elem *, void *), void *arg)
{
  if(e && fonc) {
    ItererElem(e, fonc, arg);
    fonc(e, arg);
  }	
}
/***************************************************/
