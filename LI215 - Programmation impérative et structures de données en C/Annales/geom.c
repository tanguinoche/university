...
Un_noeud *AjouterPoint(Un_noeud *abr, Un_point *p){
  if(AjouterABR(&abr,p->id,p)==NULL){
    printf("Id %d incorrect. (AjouterPoint \n",p->id);
    return NULL;
  }
  return abr;
}	
...
{
  Une_forme *f =(Une_forme *) malloc(sizeof(Une_forme));
  if(f == NULL) {
...
f->type = t;
f->coul = c;
 f->tabp = (Un_point **)malloc(sizeof(Un_point*)*nb_points);
 if(f->tabp == NULL) {
...
Un_elem *CreerElem(Une_forme *f)
{
  ...
  Un_elem *el = (Un_elem *) malloc(sizeof(Un_elem));
  if(!el) {
...
{
  Un_dessin *d = (Un_dessin *) calloc(sizeof(Un_dessin), 1);
  if(d == NULL) {
